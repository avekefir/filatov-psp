...
const PRESETS = [
  { id: 1, title: "Ноутбук", model: "software_models/MacBook Pro.glb" },
  { id: 3, title: "Системник", model: "software_models/Gaming Computer.glb" },
  { id: 4, title: "Второй ноутбук", model: "software_models/test1.glb" },
  { id: 5, title: "Системник + Ноутбук", models: [
      { model: "software_models/Gaming Computer.glb" },
      { model: "software_models/MacBook Pro.glb" },
    ]
  }
];
...
function addCard(model, isUser, userId) {
  ...
  card.onclick = () => {
    if (isUser) {
      window.location.href = `software_detail.html?user=${userId}`;
    } else {
      window.location.href = `software_detail.html?id=${model.id}`;
    }
  };
